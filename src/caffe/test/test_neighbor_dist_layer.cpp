...
namespace caffe {
	...
	TYPED_TEST(NeighborDistLayerTest, TestForward_1)
	{
		...
		{
			if (std::abs(std::numeric_limits<Dtype>::max() -this->blob_top_0->cpu_data()[i]) < 10e-5)
			{
				EXPECT_NEAR(this->blob_top_0->cpu_data()[i], std::numeric_limits<Dtype>::max(), min_precision);
			}
			else
			{
