...
namespace caffe {
		...
		const vector<Blob<Dtype>*>& top)
	{
		...
		case KSparseParameter_SparseType_L1:
			decay = (Dtype)k_sparse_param.decay();
			break;
		...
		const vector<Blob<Dtype>*>& top)
	{
		...
		case KSparseParameter_SparseType_KSPARSE:
			...
			case KSparseParameter_SparseMethod_CHANNEL:
				...
				stride_ch_r = rank_val_.offset(0, 1);
		...
		const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom)
	{
		...
		case KSparseParameter_SparseType_L1:
			...
			//caffe_cpu_sign(top[0]->count(), top[0]->cpu_data(), tmp_.mutable_cpu_data());
			//caffe_axpy(bottom[0]->count(), decay, tmp_.cpu_data(), bottom_diff);
			caffe_cpu_sign(top[0]->count(), top[0]->cpu_data(), bottom_diff);
			caffe_cpu_axpby(bottom[0]->count(), (Dtype)1, top_diff, decay, bottom_diff);
			break;
		case KSparseParameter_SparseType_L2:
			caffe_copy(bottom[0]->count(), top_diff, bottom_diff);
			caffe_axpy(bottom[0]->count(), decay, top[0]->cpu_data(), bottom_diff);
			break;
		case KSparseParameter_SparseType_KSPARSE:
			switch (this->layer_param_.k_sparse_param().sparse_type())
			{
