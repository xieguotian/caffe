...
void MultiTLossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void MultiTLossLayer<Dtype>::Reshape(
  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void MultiTLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
   const vector<Blob<Dtype>*>& top) {
  LOG(ERROR) << "Forward_cpu not implemented.";
}

template <typename Dtype>
void MultiTLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  LOG(ERROR) << "Backward_cpu not implemented.";
}

#ifdef CPU_ONLY
STUB_GPU(MultiTLossLayer);
#endif

INSTANTIATE_CLASS(MultiTLossLayer);
REGISTER_LAYER_CLASS(MultiTLoss);
