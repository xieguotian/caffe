...
void NormalizationLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i=0; i<n; ++i) {
	...
	//LOG(INFO) << "norm: " << normsqr;
    caffe_gpu_scale<Dtype>(d, pow(normsqr+1e-18, -0.5), bottom_data+i*d, top_data+i*d);
  }
/*
...
void NormalizationLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i=0; i<n; ++i) {
    ...
    caffe_gpu_scale(d, Dtype(pow(a+1e-18, -0.5)), bottom_diff+i*d, bottom_diff+i*d);
  }
/*
