...
namespace caffe {
	...
	TYPED_TEST(Col2ImgMaskLayerTest, TestForward_1) {
		...
		caffe_set(this->blob_bottom_->count(), (Dtype)3, this->blob_bottom_->mutable_cpu_data());
		...
		for (int i = 0; i < this->blob_top_->count(); i++)
			EXPECT_NEAR(this->blob_top_->mutable_cpu_data()[i], 3, min_precision);
	}
	...
	TYPED_TEST(Col2ImgMaskLayerTest, TestForward_2) {
		...
		convolution_param->add_kernel_size(3);
		convolution_param->add_stride(2);
		caffe_set(this->blob_bottom_->count(), (Dtype)2, this->blob_bottom_->mutable_cpu_data());

		Blob<Dtype> mask;
		mask.ReshapeLike(*this->blob_bottom_);
		caffe_set(mask.count(), (Dtype)1, mask.mutable_cpu_data());
		vector<Blob<Dtype>*> blob_bottom_vec_2_;
		blob_bottom_vec_2_.push_back(this->blob_bottom_);
		blob_bottom_vec_2_.push_back(&mask);

		Col2imgMaskLayer<Dtype> layer(layer_param);
		layer.SetUp(blob_bottom_vec_2_, this->blob_top_vec_);
		...
		const Dtype min_precision = 1e-5;
		for (int i = 0; i < this->blob_top_->count(); i++)
			EXPECT_NEAR(this->blob_top_->mutable_cpu_data()[i], 2, min_precision);
	}
