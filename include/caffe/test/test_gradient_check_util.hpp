...
class GradientChecker {
 public:
  ...
  void CheckGradientExhaustive2(Layer<Dtype>* layer,
	  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top,
	  int check_bottom = -1);
  void CheckGradientSingle2(Layer<Dtype>* layer,
	  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top,
	  int check_bottom, int top_id, int top_data_id, bool element_wise = false);
 protected:
...
template <typename Dtype>
void GradientChecker<Dtype>::CheckGradientExhaustive2(Layer<Dtype>* layer,
	const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top,
	int check_bottom) {
	//layer->SetUp(bottom, top);
	CHECK_GT(top.size(), 0) << "Exhaustive mode requires at least one top blob.";
	...
	for (int i = 0; i < top.size(); ++i) {
		...
		for (int j = 0; j < top[i]->count(); ++j) {
			// LOG(ERROR) << "Exhaustive: blob " << i << " data " << j;
			CheckGradientSingle2(layer, bottom, top, check_bottom, i, j);
		}
	}
}

template <typename Dtype>
void GradientChecker<Dtype>::CheckGradientSingle2(Layer<Dtype>* layer,
	const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top,
	int check_bottom, int top_id, int top_data_id, bool element_wise) {
	if (element_wise) {
		CHECK_EQ(0, layer->blobs().size());
